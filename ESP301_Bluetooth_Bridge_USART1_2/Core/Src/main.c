...
IWDG_HandleTypeDef hiwdg; /* Adicionado para o Watchdog */
...
static void MX_IWDG_Init(void); /* Adicionado para o Watchdog */
/* USER CODE BEGIN PFP */
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  MX_USART2_UART_Init();
  MX_IWDG_Init(); /* Inicializa o Watchdog */
  /* USER CODE BEGIN 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    HAL_IWDG_Refresh(&hiwdg); // Alimenta o Watchdog para evitar reset
  }
  /* USER CODE END 3 */
}
...
{
  hiwdg.Instance = IWDG;
  hiwdg.Init.Prescaler = IWDG_PRESCALER_32; // Exemplo: ~1 kHz
  hiwdg.Init.Reload = 1000; // Exemplo: ~1 segundo até reset
